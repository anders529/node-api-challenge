...
router.get('/', (req, res) => {
    db.get()
        .then(action => {
            if (action) {res.status(201).json(action);}
            else {res.status(500).json({errorMessage:'The action information could not be retrieved.'});}
        })
        .catch(error => {
            console.log(error);
            res.status(500).json({});
...
router.get('/:id', validateActionId, (req, res) => {
    ...
    db.get(id)
        .then(action => {
            if (action) {res.send(action);}
            else {res.status(404).json({message:'The action with the specified ID does not exist.'});}
        })
        .catch(err => {
...
router.put('/:id', validateActionId, (req, res) => {
    ...
    db.update(id, changes)
        .then(action => {
            if (!action) {res.status(404).json({message:'The action with the specified ID does not exist.'});}
            else if (changes.description && changes.notes) {res.json(changes);}
            else {
